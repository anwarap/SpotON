...
function validateCode(){
    ...
    if(!code.match(codeRegex)){
        codeError.innerHTML = 'Code must be minimum - 5  letters'
        return false
    }
    codeError.innerHTML = ''
    return true
...
function validateMaxDiscount(){
    ...
    if(maxDiscount < 1000 ){
        maxDiscountAmountError.innerHTML = 'Max Discount must be greater than or equal to 1000'
        return false
    }
...
function validateMinPurchase(){
    ...
    if(minPurchase <= 0){
        minPurchaseError.innerHTML = 'Min Purchase amount required must be  >=500'
        return false
